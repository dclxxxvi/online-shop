...
const ProductsPage: React.FC = () => {
	...
	const { products, isLoading, total, fetchProducts, deleteProduct, hasNext } = useProductStore();
	...
	const loadMore = useCallback(() => {
		if (storeId) {
			fetchProducts(storeId, debouncedSearch || undefined);
		}
	}, [storeId, debouncedSearch, fetchProducts]);

	const lastElement = useVirtualized({
		loadMore,
		hasNext,
		loading: isLoading,
		rootMargin: '200px',
			...
			<AlertDialog open={!!deletingProduct} onOpenChange={() => setDeletingProduct(null)}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Вы уверены, что хотите удалить товар "{deletingProduct?.name}"? Это действие нельзя
							отменить.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
