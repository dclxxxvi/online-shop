...
export const useOrderStore = create<OrderState>((set) => ({
	...
	fetchOrders: async (storeId: string, page = 1) => {
		...
		try {
			const response = await apiClient.get<PaginatedResponse<Order>>(`/stores/${storeId}/orders`, {
				page,
				limit: 10,
			});
			set({
	...
	updateOrderStatus: async (storeId: string, id: string, status: OrderStatus) => {
		...
		try {
			const updatedOrder = await apiClient.patch<Order>(`/stores/${storeId}/orders/${id}/status`, {
				status,
			});
			set((state) => ({
