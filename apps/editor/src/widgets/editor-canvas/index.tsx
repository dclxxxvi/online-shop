	...
	return (
		<main
			className={`flex-1 overflow-y-auto transition-all duration-300 ${
				isOver ? 'bg-emerald-50/50' : 'bg-gray-100'
			}`}
		>
			<div
				ref={setNodeRef}
				onClick={handleCanvasClick}
				className="max-w-4xl mx-auto my-6 bg-white min-h-[calc(100%-3rem)] shadow-sm"
			>
				{blocks.length === 0 ? (
					<div
					...
					>
						<div
							className={`text-center p-8 rounded-2xl border-2 border-dashed transition-all duration-300 ${
								isOver ? 'border-emerald-400 bg-emerald-100/50 scale-105' : 'border-gray-300'
							}`}
						>
							<div
							...
							<p className="text-sm opacity-70">
								{isOver
									? 'Блок будет добавлен на страницу'
									: 'или выберите шаблон для быстрого старта'}
							</p>
						</div>
					</div>
				) : (
