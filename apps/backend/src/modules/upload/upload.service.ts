...
export class UploadService {
	...
	private async generatePresignedUrl(file: PresignFileDto): Promise<PresignedFile> {
		...
		if (file.size > MAX_SIZE) {
			throw new BadRequestException(`Файл "${file.name}" превышает максимальный размер 5 МБ`);
		}
	...
	async getResizedImage(
		...
		try {
			await this.s3.send(new HeadObjectCommand({ Bucket: this.bucket, Key: cacheKey }));
